...
exports.SUPPORTED_TYPES = void 0;
...
function extractFormFields(text) {
    ...
    if (classSection) {
        ...
        if (hasBasic) {
            fields.classification_type = "research";
            fields.class_input = "basic";
        }
        else if (hasApplied) {
            fields.classification_type = "research";
            fields.class_input = "applied";
        }
        else if (hasPilot) {
            fields.classification_type = "development";
            fields.class_input = "pilot_testing";
        }
    ...
    if (budgetSection) {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            if (/^(PS|MOOE|CO|TOTAL|Year|Source|Of\s+funds)/i.test(line))
                continue;
